...
**Session tokens** (`art_live_xxx`) are the agent's runtime credential. 24-hour TTL, with refresh available from the dashboard before expiry. Scoped to a single agent. Cannot be extended by the agent or used to access other agents. If a token is compromised, the damage is bounded to a 24-hour window — and you can revoke it instantly from the dashboard, or revoke the underlying attestation from your wallet for a permanent kill.
