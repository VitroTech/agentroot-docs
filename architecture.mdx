...
AgentRoot separates the Owner's control surface from the agent's runtime surface. The two planes share no credentials, no sessions, and no privileges. An attacker who compromises one plane cannot reach the other.

```mermaid
flowchart TB
    subgraph CP["Control Plane (Owner)"]
        DSH[Dashboard]
        POL[Policy Engine]
...
**Session tokens** (`art_live_xxx`) are the agent's runtime credential. Long-lived — no timed expiry, no rotation; set once in the agent's environment. Scoped to a single agent. Cannot be extended by the agent or used to access other agents. If a token is compromised, you cut access by rolling the token or freezing/killing the agent — revocation is enforced on every request, so the exposure window is how fast you revoke, not a fixed lifetime.
...
- The Owner's wallet address (or AgentRoot's managed-attester wallet, in social-login mode)
...
| Session token rolled | `401 TOKEN_REVOKED` | A new token was issued; the old value is rejected |
...
If the agent's environment is fully exposed, the attacker has proxy access with the agent's existing bindings only until you cut it off — roll the token or freeze the agent from the dashboard in seconds, or kill the attestation permanently from your wallet. Revocation is enforced on every request.

## Related

- **[Introduction](introduction)** — the Pinocchio Problem, the action-class thesis, and why onchain matters
- **[Lifecycle](lifecycle)** — Mint, Freeze, Kill, ReMint
