...
The [lifecycle](lifecycle) is the noun system — Mint, Freeze, Kill, ReMint. The kill switch is the verb system — *how* each transition is enforced when you click the button. This page covers the four mechanisms, what they cost, what they cannot do, and what your agent sees when one fires.

If you want scenarios — when to reach for which mechanism — read [Owner Playbooks](playbooks) first.

## The four mechanisms

| Mechanism | Lifecycle | Scope | Speed | Reversible | Requires |
|---|---|---|---|---|---|
| **Tier 1** | Freeze (key) | One binding | Instant | Yes | Dashboard |
| **Tier 2** | Freeze (agent) | Whole agent | Instant | Yes | Dashboard |
| **Tier 3** | Kill | Onchain | next few blocks (managed adds an instant freeze leg) | No (recoverable via ReMint) | Wallet |
| **Auto-kill** | Freeze (auto) | Threshold-based | Instant | Yes (reset) | Pre-configured |

Tier 1, Tier 2, and Auto-kill are all forms of Freeze — they flip a flag the proxy checks on every request. Tier 3 is the only mechanism that touches onchain state; it is the sovereign kill, and it works without AgentRoot's infrastructure.

## Tier 1 — freeze one API binding
...
**Where the flag lives:** an edge-cached flag, refreshed continuously. Updates propagate globally in under five seconds.
...
For multi-agent shutoffs (one provider for *every* agent), Freeze the *key* instead — see [Lifecycle → Keys](lifecycle).

## Tier 2 — freeze the whole agent
...
**Where the flag lives:** same edge cache, separate flag. Same global propagation window.
...
**What it does:** sets the EAS attestation's `revocationTime` to the current block timestamp on Base. The proxy checks attestation status on every request and returns `403 ATTESTATION_REVOKED` for any agent whose attestation is revoked.

**Two-phase when AgentRoot orchestrates the Kill.** A Kill issued *through* AgentRoot — the dashboard or the 3AM button — is two-phase: an immediate **Freeze** (the proxy denies the agent's very next call, sub-second, with no wait on the chain) followed by the **onchain revocation**, which confirms in the next few blocks as the permanent, anyone-can-verify receipt. In pure **sovereign** mode — your wallet revoking directly against the EAS contract, AgentRoot out of the loop — there is no AR Freeze leg; the proxy enforces the revocation on the agent's next call once it is visible onchain. Either way: instant protection, durable certainty.

**The critical property:** the revocation transaction is between *your wallet* and *the EAS contract on Base*. AgentRoot is not in the loop. You don't need our dashboard, our API, or our servers. If you have your wallet and a way to talk to Ethereum, you can kill the agent — even if AgentRoot has gone dark.
...
**What it does:** the proxy maintains per-agent and per-key counters at the edge. When a counter exceeds a configured threshold, the proxy automatically sets the Tier 2 flag on the offending agent (or the equivalent key-level flag) and returns `429 RATE_LIMITED` on subsequent calls.
...
Handle them at the HTTP layer — the JSON body's `error.code` is the discriminator:

```typescript
const res = await fetch(`${process.env.AGENTROOT_PROXY}/openai/v1/chat/completions`, {
  method: 'POST',
  headers: {
...
if (!res.ok) {
  ...
  switch (error.code) {
    ...
    case 'PROVIDER_DISABLED':
      ...
      break;
    case 'RATE_LIMITED':
      await backoffAndRetry(res.headers.get('Retry-After'));
      break;
  }
}
```

See the [API Reference](api-reference) for the full error catalog.
...
flowchart TD
    ...
    Q1 -->|Yes| T1[Tier 1<br/>Freeze binding]
    Q1 -->|No, whole agent| Q2{Need permanent<br/>shutdown of<br/>this identity?}
    Q2 -->|No, investigate first| T2[Tier 2<br/>Freeze agent]
...
For the Owner-facing version of this — when to use each in real scenarios — see [Owner Playbooks](playbooks).
...
- **[Owner Playbooks](playbooks)** — when to reach for which kill, in real scenarios
